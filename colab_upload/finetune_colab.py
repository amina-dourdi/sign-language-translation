...
CONFIG = {
    ...
    # Fine-tuning hyperparameters (V2 - HIGH PRECISION)
    "freeze_encoder": False,   # Toujours False pour le Fine-Tuning complet
    "lr": 5e-5,                
    "weight_decay": 1e-2,      
    "epochs": 50,
    "batch_size": 8,
...
def finetune():
    print("=" * 60)
    print("   CSLT FINE-TUNING PIPELINE (V2)")
    print("   (Transfer Learning - High Precision)")
    print("=" * 60)
    device = torch.device(CONFIG["device"])
    print(f"  Device: {device}")

    # ═══════════════════════════════════════════════════════
    # 0. Path Correction for Colab (V2.2 - CLEAN START)
    # ═══════════════════════════════════════════════════════
    print("\n[0/7] Correcting paths for Colab...")
    colab_kp_dir = "/content/data/keypoints"
    
    if not os.path.exists(colab_kp_dir):
        print(f"❌ ERROR: {colab_kp_dir} not found!")
        return
    ...
    for cid in meta:
        filename = os.path.basename(meta[cid]["npy_path"])
        meta[cid]["npy_path"] = os.path.join(colab_kp_dir, filename)
    
    fixed_meta_path = "/content/cslt/metadata_colab.json"
    with open(fixed_meta_path, "w") as f:
        json.dump(meta, f)
    
    CONFIG["metadata_path"] = fixed_meta_path
    print(f"  ✅ Metadata updated for: {colab_kp_dir}")
    ...
    # AJOUT V2 : Scheduler pour baisser le LR quand on stagne
    ...
    print("─" * 75)
    print(f"  {'Epoch':>5} │ {'TrLoss':>7} {'VaLoss':>7} │ {'TrAcc':>6} {'VaAcc':>6} │ {'BLEU4':>6} │ {'Time':>4} │ {'Status'}")
    print("─" * 85)
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        # --- AJOUT V2 : Calcul BLEU rapide sur 100 samples de validation ---
        all_preds_val, all_refs_val = [], []
        with torch.no_grad():
            for i, (kp, tgt, _, _) in enumerate(va_loader):
                if i * CONFIG["batch_size"] >= 100: break # Limite à 100 pour la vitesse
                kp = kp.to(device)
                preds = model.translate(kp, CONFIG["max_seq_len"], 1.3, tok)
                all_preds_val.extend(preds)
                for j in range(tgt.size(0)):
                    all_refs_val.append(tok.decode(tgt[j].tolist()) if hasattr(tgt[j], 'tolist') else tok.decode(tgt[j]))
        
        val_bleu = compute_bleu(all_preds_val, all_refs_val)["bleu4"]
        # ------------------------------------------------------------------
        ...
        # AJOUT V2 : Mise à jour du scheduler
        ...
        print(f"  {epoch:>3}/{CONFIG['epochs']} │ {tr_loss:>7.4f} {val_loss:>7.4f} │ {tr_acc:>5.1f}% {val_acc:>5.1f}% │ {val_bleu:>6.2f} │ {elapsed:>3.0f}s │ {status} (lr={current_lr:.1e})")
